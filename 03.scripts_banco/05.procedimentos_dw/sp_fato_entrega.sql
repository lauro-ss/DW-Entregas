...
CREATE OR ALTER PROCEDURE SP_Fato_Status_Regiao_Mes(@DATA_CARGA DATETIME) AS
BEGIN
	
	DECLARE @QUANTIDADE INT, @STATUS VARCHAR(20), @REGIAO VARCHAR(20), @MES VARCHAR(20),
			@STATUS_ID INT, @REGIAO_ID INT, @MES_ID INT

	DECLARE C_FATO_STATUS_REGIAO_MES CURSOR FOR SELECT SUM(E.quantidade), S.status, L.regiao, T.nome_mes FROM Fato_Entrega E 
	...
	OPEN C_FATO_STATUS_REGIAO_MES
	FETCH C_FATO_STATUS_REGIAO_MES INTO @QUANTIDADE, @STATUS, @REGIAO, @MES
	WHILE(@@FETCH_STATUS = 0)
	BEGIN
		SET @STATUS_ID = (SELECT ID FROM Dim_Status WHERE status = @STATUS)
		SET @REGIAO_ID = (SELECT top 1 id FROM Dim_Localidade WHERE regiao = @REGIAO)
		SET @MES_ID = (SELECT ID FROM Dim_Tempo T WHERE T.nivel = 'MES' AND T.nome_mes = @MES AND T.ano = YEAR(@DATA_CARGA))
		
		INSERT INTO Fato_Status_Regiao_Mes VALUES (@MES_ID, @REGIAO_ID, @STATUS_ID, @QUANTIDADE)

		FETCH C_FATO_STATUS_REGIAO_MES INTO @QUANTIDADE, @STATUS, @REGIAO, @MES
	END
	CLOSE C_FATO_STATUS_REGIAO_MES
	DEALLOCATE C_FATO_STATUS_REGIAO_MES
END
...
EXEC SP_Fato_Status_Regiao_Mes '20221106'
SELECT * FROM Vio_Entrega
SELECT * FROM Fato_Status_Regiao_Mes